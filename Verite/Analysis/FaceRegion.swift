...
enum FaceRegion: String, CaseIterable, Sendable {
    ...
    case nose
    case chin
    /// The band between the eye bottoms and the nose bridge. Used exclusively
    /// for the dark-circle estimate; skipped in all standard aggregations.
    case underEye
    ...
    /// Exclude under-eye from standard aggregation loops (it's only used
    /// by `ExtendedSkinMetrics.darkCircles`).
    var isStandardRegion: Bool { self != .underEye }
    ...
    var fractionalRect: (x0: Double, y0: Double, x1: Double, y1: Double) {
        ...
        case .underEye:   return (0.25, 0.30, 0.75, 0.42)
        }
    }
}
