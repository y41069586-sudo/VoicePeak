...
1. **Face photos never leave the device.** Only local thumbnail *filenames* are
   ...
   `CloudSkin` module: `FeatureFlags.cloudSkinAnalysisEnabled` defaults to true,
   but it stays fully inert — no photo ever uploaded — until a real API key is
   also present at build time (`DermIQConfig.isConfigured`), which nothing in
   this repo ships configured. See `CloudSkin/README.md`.
...
| `CloudSkin/` | `CloudSkinAnalysisService` protocol, `DermIQClient` (flag on, inert until a key is configured) — uploads the photo, unlike every other module | 12 |
